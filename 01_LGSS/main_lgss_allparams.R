...
if (plot_likelihood_surface) {
  ...
  for (i in 1:length(phi_grid)) {
    ...
    # } else {
    kf_out <- compute_kf_likelihood(
      state_prior_mean = state_ini_mean,
...
if (rerun_rvgaw) {
  rvgaw_results <- run_rvgaw_lgss(
    y = y, # sigma_eta = sigma_eta, sigma_eps = sigma_eps,
...
if (rerun_hmc) {
  ...
  fit_stan_lgss <- lgss_model$sample(
    ...
    refresh = 100,
    iter_warmup = burn_in,
    iter_sampling = hmc_iters,
    save_warmup = TRUE
  )

  hmc_results <- list(
    draws = fit_stan_lgss$draws(
      variables = c("phi", "sigma_eta", "sigma_eps"),
      inc_warmup = TRUE
    ),
...
if (rerun_hmcw) {
  ...
  fit_stan_lgss_whittle <- whittle_lgss_model$sample(
    ...
    refresh = 100,
    iter_warmup = burn_in,
    iter_sampling = hmcw_iters, # n_post_samples / n_chains,
    save_warmup = TRUE
  )

  hmcw_results <- list(
    draws = fit_stan_lgss_whittle$draws(
      variables = c("phi", "sigma_eta", "sigma_eps"), 
      inc_warmup = TRUE
      ),
    time = fit_stan_lgss_whittle$time(),
    summary = fit_stan_lgss_whittle$summary(variables = c("phi", "sigma_eta", "sigma_eps")),
    metadata = fit_stan_lgss_whittle$metadata()
  )
