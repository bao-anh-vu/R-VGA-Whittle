compute_grad <- tf_function(
    compute_grad <- function(samples_tf, I_i, freq_i, blocksize, noise_dist = "gaussian") {
        log_likelihood_tf <- 0
        with(tf$GradientTape() %as% tape2, {
            with(tf$GradientTape(persistent = TRUE) %as% tape1, {
                ...
                if (noise_dist == "t") {
                    ...
                    ## Gaussian noise
                    spec_dens_eps_tf <- nu_s^2 #tf$math$exp(samples_tf[, 5])
                }
        ...
        return(list(
            ...
            term2 = term2,
            term3 = term3,
            spec_dens_x = spec_dens_x_tf,
            spec_dens_eps = spec_dens_eps_tf,
...
compute_grad_logit <- tf_function(
    compute_grad_logit <- function(samples_tf, I_i, freq_i, blocksize, noise_dist = "gaussian") {
        log_likelihood_tf <- 0
        with(tf$GradientTape() %as% tape2, {
            with(tf$GradientTape(persistent = TRUE) %as% tape1, {
                S <- as.integer(nrow(samples_tf))
                ...
                phi_s <- tf$math$divide(tf$math$exp(samples_tf[, 1]), (1 + tf$math$exp(samples_tf[, 1])))
                phi_s <- tf$reshape(phi_s, c(length(phi_s), 1L, 1L)) # S x 1 x 1

                theta_s <- tf$math$divide(tf$math$exp(samples_tf[, 2]), (1 + tf$math$exp(samples_tf[, 2])))
                theta_s <- tf$reshape(theta_s, c(length(theta_s), 1L, 1L)) # S x 1 x 1
